...
(function(){
  ...
  function startRound(){
    ...
    if(Array.isArray(allData) && allData.length){
      renderFromData(randomSample(allData, ROUND_SIZE));
    } else { initMailItems(); }
  }
  ...
  bin?.addEventListener('drop', (e)=>{
    ...
    if(item.el.parentElement !== binList){ binList?.appendChild(item.el); item.inBin = true; }
  });
  ...
  list?.addEventListener('drop', (e)=>{
    ...
    const item = mails.find(m => m.id === id);
    if(!item) return;
    if(item.el.parentElement !== list){ list.appendChild(item.el); item.inBin = false; }
  });
    ...
    for(const url of candidates){
      try{
        ...
        if(Array.isArray(data) && data.length){
          allData = data.map(d=>({ id:d.id, text:d.text, type: normalizeType(d.type), emoji:d.emoji, aria:d.aria }));
          startRound();
          loaded = true;
          break;
        }
  ...
  function runConfetti(){
    const prep = prepareCanvas(); if(!prep) return; const { w, h } = prep; ctx = prep.ctx;
  ...
  function runBoom(){
    ...
    const step = ()=>{
      ctx.clearRect(0,0,w,h);
      pieces.forEach(p=>{
        p.x += p.vx; p.y += p.vy; p.vy += 0.04; p.rot += p.vr; p.life -= 1; p.alpha = Math.max(0, p.life/90);
